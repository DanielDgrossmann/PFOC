...
# Retangular
    ...
    while True:
        if b1 == 'n':
            ...
            b = input('Digite o valor da base (14 cm a 25 cm e 30 cm): ').strip().lower()

            if b.isnumeric() and (14 <= int(b) <= 30):

                if int(b) != 26 and int(b) != 27 and int(b) != 28 and int(b) != 29:
                    break
                else:
        ...
        if h1 == 'n':
            ...
            if h.isnumeric() and (19 <= int(h) <= 150):
                ...
                if b.isnumeric() and int(h) * int(b) <= 360:
                    ...
                    if (int(b) != 19 and int(b) != 21 and int(b) != 22 and int(b) != 23 and b != 24) \
                            and int(h) % 5 != 0:
                        v = False
                ...
                for i3 in range(0, len(bitolas_iniciais)):
                    if bitolas_iniciais[i3][0] <= fi_l_max:
                        ...
                        Nx_max = max(floor((base - 2 * (c_nom + fi_t / 10) + emin_livre + fi_l / 10) /
                                           (emin_livre + fi_l / 10 + 2 * fi_l / 10)), 2)
...
if geometria[0] == 'r':
    ...
    for i1 in range(0, len(tabela_solucoes_possiveis)):
        ...
        for i2 in range(0, len(situacoes_de_calculo)):
            ...
            razao = circular.verificacao(fck, fi_pilar, cobrimento, int(Nc), fi_t, fi_t, Nsd, Msd_x, Msd_y, Aso)
            # fck, fi_pilar, c, Nc, fi_t, fi_l, Nd, Mdx, Mdy, Aso, relatorio = False
        ...
        if numero_verificacoes == len(situacoes_de_calculo):
            ...
            for i2 in range(0, len(situacoes_de_calculo)):
                ...
                print(f'\n\033[1;34m{texto:}\033[m')
                circular.verificacao(fck, fi_pilar, cobrimento, int(Nc), fi_t, fi_t, Nsd, Msd_x, Msd_y, Aso,
                                     relatorio=True)
            break
    ...
    if not ha_respostas:
        print(f"Sem soluções possíveis.\n")
